...
import { authorize } from "./userSlice";
...
export const me = createAsyncThunk(
  ...
  async (_, { rejectWithValue, dispatch }) => {
    try {
      ...
      if (response.data) {
        dispatch(authorize(response.data.auth.me));
...
export const login = createAsyncThunk(
  ...
  async (params: LoginRequest, { rejectWithValue, dispatch }) => {
    try {
      ...
      if (response.data) {
        dispatch(authorize(response.data.auth.login));
...
export const register = createAsyncThunk(
  ...
  async (params: RegisterRequest, { rejectWithValue, dispatch }) => {
    try {
      ...
      if (response.data) {
        dispatch(authorize(response.data.auth.register));
