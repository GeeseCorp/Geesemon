...
export const ContextMenu: FC<Props> = ({ children, items }) => {
  ...
  const [translateX, setTranslateX] = useState(true);
  const [translateY, setTranslateY] = useState(true);

  const onRightClick = (x: number, y: number) => {
    setOpen(true);
    setLocation({ x, y });

    setTranslateX(document.body.clientWidth / 2 < x);
    setTranslateY(document.body.clientHeight / 2 < y);
  };
        ...
        <AnimatePresence>
          <div
            ...
            style={{
              left: location.x,
              top: location.y,
              transform: `translate(${translateX ? '-100%' : '0'}, ${translateY ? '-100%' : '0'})`,
            }}
          >
            <motion.div
